...
"@(#) $Header: /cvsroot/nsnam/ns-2/tcp/tcp-dc-vegas.cc,v 1.37 2005/08/25 18:58:12 johnh Exp $ (NCSU/IBM)";
#endif
...
static class DCVegasTcpClass : public TclClass {
public:
	DCVegasTcpClass() : TclClass("Agent/TCP/DCVegas") {}
	TclObject* create(int, const char*const*) {
		return (new DCVegasTcpAgent());
	}
} class_dcvegas;


DCVegasTcpAgent::DCVegasTcpAgent() : TcpAgent()
{
	v_sendtime_ = NULL;
	v_transmits_ = NULL;

	//ackHolder = NULL;
	//ackHolder = new int[maxcwnd_];
}

DCVegasTcpAgent::~DCVegasTcpAgent()
{
	if (v_sendtime_)
		delete []v_sendtime_;
	if (v_transmits_)
...
void
DCVegasTcpAgent::delay_bind_init_all()
{
...
int
DCVegasTcpAgent::delay_bind_dispatch(const char *varName, const char *localName, 
				   TclObject *tracer)
{
	/* init vegas var */
...
void
DCVegasTcpAgent::recv_newack_helper(Packet *pkt)
{
	newack(pkt);
#if 0
...
void
DCVegasTcpAgent::recv(Packet *pkt, Handler *)
{
	...
	//c2
	// checking if all the packets in the current window have been acknowledged
	// if yes, update F_dcv and a_dcv
	int tempCwnd = (int) cwnd_;
	int idx = tcph->seqno() - tempCwnd;
	int ackInWIndow = nackpack_ - ackHolder[idx] ; // number of packets acked in current window
	if(ackInWIndow == tempCwnd) {
		// all packets in current window  have been acked
		if(ackInWIndow != 0) {
			F_dcv = totalACKAbove * 1.0 / ackInWIndow;
        	double g = 0.05;
			a_dcv = (1 - g) * a_dcv + g * F_dcv;
			totalACKAbove = 0;
		}
	}
	Packet::free(pkt);
...
void
DCVegasTcpAgent::timeout(int tno)
{
...
void
DCVegasTcpAgent::output(int seqno, int reason)
{
	Packet* p = allocpkt();
	...
	if (seqno==0) {
		...
		// ackHolder = new int[maxcwnd_ * 10000];
	...
	send(p, 0);
	ackHolder[seqno] = (int) nackpack_; // number of acks upto this packet //c2
 ...
 */
int
DCVegasTcpAgent::vegas_expire(Packet* pkt)
{
	hdr_tcp *tcph = hdr_tcp::access(pkt);
