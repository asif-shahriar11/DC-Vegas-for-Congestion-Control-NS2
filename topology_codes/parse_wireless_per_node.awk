...
#Calculating residual energy
	for(i=0; i<maximumNodeNumber; i++){
		...
		if(residual_energy[i] < 0) printf("neg res\n") >> "neg.txt";
	}
...
#For calculations of throughput
	...
	for(i=0; i<maximumNodeNumber; i++){
		...
		if(throughput_per_node_bits[i] < 0) printf("negative 1 --------------------\n") >> "neg.txt";
	...
	for(i=0; i<maximumNodeNumber; i++){
		if(initial_total_energy[i] > 0){
			...
			printf("%d\n%f %f ", i, energy_consumption[i], residual_energy[i]) >> "output_per_node.txt";
			# printf("Node %d, Residual Energy (J) = %f\n", i, residual_energy[i]);
		}
		...
		if(start_time_per_node[i] != 100000 && throughput_per_node_bits[i] > 0){
			...
			printf("%f\n",throughput_per_node_bits[i]) >> "output_per_node.txt";
...
printf("Total_energy: %f\nAvg energy per packet: %f\n", total_energy_consumption, avg_energy_per_packet); 
